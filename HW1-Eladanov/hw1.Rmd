...
```{r}
passwords <- read_csv("https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2020/2020-01-14/passwords.csv",col_types = cols())
```
...
```{r}
passwords %>% filter(category == "fluffy")
```

Filter only passwords in the "sport" and "cool-macho" and select only the `rank`, `password`, `category` and `strength` columns:

```{r}
passwords %>% filter(category == "sport" | category == "cool-macho") %>% select(rank, password, category, strength)
```

Same as above, but sort by `strength`, from weakest to strongest (and do try to make this in a single pipe):

```{r}
passwords %>% filter(category == "sport" | category == "cool-macho") %>% select(rank, password, category, strength) %>% arrange(strength)
```
...
```{r}
passwords_with_123 <- passwords %>% filter(is_123) %>% pull(password)
...
passwords %>% 
  filter(category == "food") %>% 
    summarise(avg_strength_of_food = mean(strength,na.rm = TRUE))
```
...
passwords %>% 
  group_by(category) %>% 
  summarise(n = n(), mean_strength = mean(strength, na.rm = TRUE)) %>% arrange(-mean_strength)
```
...
passwords <- passwords %>%
  mutate(time_unit_seconds = case_when(
    str_detect(time_unit, "second") ~ 1,
    str_detect(time_unit,"minute") ~ 60,
    str_detect(time_unit, "hour") ~ 3600,
    str_detect(time_unit, "day") ~ 86400,
    str_detect(time_unit, "week") ~ 604800,
    str_detect(time_unit, "month") ~ 2629746,
    str_detect(time_unit, "year") ~ 31536000,
    TRUE ~ NA_real_
  ))
```
...
```{r}
passwords <- passwords %>%
  mutate(time_unit_seconds = case_match(time_unit,
    "seconds" ~ 1,
    "minutes" ~ 60,
    "hours" ~ 3600,
    "days" ~ 86400,
...
```{r}
passwords <- passwords %>%
  mutate(across(c("time_to_crack_sec", "offline_crack_sec"), log, .names = "{.col}_log"))
```
...
```{r}
passwords <- passwords %>%
mutate(strength = replace(strength, strength > 10, NA))
```
...
category_mean_strength <- function(categ) {
  passwords %>%
    filter(category == categ) %>% summarise(
      avg_rank = mean(rank ,na.rm = TRUE),
      avg_strength = mean(strength, na.rm = TRUE),
      avg_time_to_crack_sec_log = mean(time_to_crack_sec_log, na.rm = TRUE)
    ) %>% 
    mutate(categ = categ) %>%
...
median_per_category <- function(numeric_var) {
  passwords %>% 
    group_by(category) %>% 
    summarise(med = median({{numeric_var}}, na.rm = TRUE))
}
...
```{r}
summary(lm_obj)$coefficients[,4]['x2']
```
...
```{r}
library(broom)
tidy(lm_obj) %>% filter(term == "x2") %>% select(p.value)
```
