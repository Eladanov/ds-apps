...
```{r}
# The Leiden method introduces two main modifications to the Louvain method.
# Firstly, before aggregating the communities, a refinement of the partition is carried out when partitioning the graph.
# Secondly, a fast local move procedure is employed to relocate nodes during the merging of communities. Unlike the Louvain method that revisits all nodes to improve the quality function, the Leiden method only revisits nodes whose neighborhoods have undergone changes. 
# These two adjustments result in faster execution times and ensure the connectivity of communities, a feature lacking in the Louvain method.
```
