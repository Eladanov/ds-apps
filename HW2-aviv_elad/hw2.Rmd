...
colnames(dirty_data) <- map(colnames(dirty_data), clean_country_name)
```
...
```{r}
dirty_data <- dirty_data %>% select(-c(4, 6, 7, 8, 9))
```
...
```{r}
dirty_data <- dirty_data %>%
  mutate(across(c(5:236),as.numeric))
```
...
tidy_up_data <- function(.sheet, .gender) {
  dirty_data <- read_excel("data/UN_MigrantStockByOriginAndDestination_2019.xlsx", sheet = .sheet, skip = 15)
    ...
    dirty_data %>%
      rename("year"=1, "order"=2, "country_dest"=3,"code"=5) %>% 
      rename_with(.fn = clean_country_name) %>% 
      filter(code < 900) %>% 
      select(-c(4, 6, 7, 8, 9)) %>%
      mutate(across(where(is.character), ~na_if(., "-"))) %>%
      mutate(across(where(is.character), ~na_if(., ".."))) %>%
...
```{r}
films_attrs <- c("title" = FALSE, "episode_id" = FALSE,"release_date" = FALSE, "characters" = TRUE, "planets" = TRUE, "starships" = TRUE, "vehicles" = TRUE)

films <- sw_films %>% map_dfr(get_sw_element_details, films_attrs)
films
```
...
orig_chars <- sw_people %>% 
  map_dfr(get_sw_element_details, char_attrs)

gen_gsub <- function(x){
  return(as.numeric(gsub("\\D","",x)))
}

chars <- sw_people %>% 
  map_dfr(get_sw_element_details, char_attrs) %>% 
  mutate(across(where(is_character), ~{case_match(.x, c("unknown", "n/a") ~ NA_character_,.default =  .x)})) %>%
  mutate(mass = as.numeric(gsub(",","",mass))) %>%
  mutate(height = as.numeric(height)) %>%
  mutate(homeworld = gen_gsub(homeworld)) %>%
  mutate(films = map(films,gen_gsub)) %>%
  mutate(birth_year = map_dbl(birth_year, parse_number)) %>%
  rowid_to_column("character_id")
chars
```
