...
```{r}
predictors <- colnames(spotify_songs2)[c(12:23)]
...
split_obj <- spotify_songs2 %>% 
  initial_split(prop = 0.8)

sptfy_tr <- training(split_obj)
sptfy_te <- testing(split_obj)
...
results_test <- mod_l1 %>% 
    ...
    mutate(truth = sptfy_te_ready$playlist_genre,method = "L1 Lasso") %>%
  bind_rows(mod_l2 %>% 
    predict(new_data = sptfy_te_ready, type = "prob") %>%
    mutate(truth = sptfy_te_ready$playlist_genre,method = "L2 Ridge")) %>%
  bind_rows(mod_rf %>%
    predict(new_data = sptfy_te_ready, type = "prob") %>%
    mutate(truth = sptfy_te_ready$playlist_genre,method = "RF")) %>%
  bind_rows(mod_ksvm %>%
    predict(new_data = sptfy_te_ready, type = "prob") %>%
    mutate(truth = sptfy_te_ready$playlist_genre,method = "SVM"))
...
results_test %>% 
  group_by(method) %>% 
  roc_auc(truth = truth, estimate = .pred_RnB, event_level = "second")
```
...
* Which is the best option giving the most realistic test MSE according to Moscovich and Rosset? Explain in short.
 - The best option is “f3”, because it follows the principle of first splitting the data to training and test sets, and defining the standardizing transformation only with the training set, and applying the same transformation to the test set as well.

* Which is the option they are most concerned with? Explain in short.
 - The option they are most concerened with is “f1”, because of 2 reasons:
      - the whole dataset is used for defining the standardizing transformation, causing data leakage.
      - splitting of the set to train and test sets is not random, thus potentially introducing bias from the distribution          to the predictor.
 
* Explain in short what's wrong with the other two options.
 -  “f2” uses the entire dataset when defining the standardizing transformation, causing data leakage.
 -  “f4” defines a different standardizing transformation for the test and training sets.
...
* Bonus 2 points: Under which assumptions/circumstances are the three options other than the best not *that* bad? (hint: you might like to read further)
 - Maybe if we already sample from a standardized distribution.
